...
`.env.example` 中各变量对应 Go worker；Python 源码保留作回滚：
...
| `AGENT_TMUX` | 仅旧 Python worker 使用；Go worker 忽略 |
...
它与 Python 版共用 `WORK_ROOT/worker.lock`，不会同时运行并认领任务。
...
回滚到 Python worker：先停止 Go 服务并核查，使用
`systemctl --user edit codex-issue-worker.service` 设置以下 drop-in（先清空 ExecStart），
保留文件与日志：
...
执行 `systemctl --user daemon-reload` 后，重新检查授权队列并启动服务。
